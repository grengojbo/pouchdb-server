...
module.exports = (startPouchDB, opts) => {
  let currentPouchDB;
  ...
  opts.overrideMode.include.forEach(function (part) {
    if (modes.fullCouchDB.indexOf(part) === -1) {
      throw new Error(
        `opts.overrideMode.include contains the unknown part '${part}'.`
      );
    }
  });

  const modeIncludes = modes[app.opts.mode || 'fullCouchDB'];
  if (!modeIncludes) {
    throw new Error(`Unknown mode: ${app.opts.mode}`);
  }
  const allIncludes = modeIncludes.concat(app.opts.overrideMode.include);
  app.includes = toObject(allIncludes);
  app.opts.overrideMode.exclude.forEach((part) => {
    if (!app.includes[part]) {
      throw new Error(
        `opts.overrideMode.exclude contains the not included part '${part}'.`
      );
    }
  ...
  app.setPouchDB = function (newPouchDB) {
    const oldPouchDB = currentPouchDB;
    currentPouchDB = newPouchDB;

    let stoppingDone = Promise.resolve();
    if (oldPouchDB) {
  ...
  app.dbWrapper.registerWrapper((name, db, next) => {
    ...
    let wrapperMethods = {};
    ['info', 'removeAttachment'].forEach((name) => {
  ...
  app.use((req, res, next) => {
    ...
    if (!currentPouchDB) {
      const msg = 'express-pouchdb needs a PouchDB object to route a request!';
      throw new Error(msg);
    }
