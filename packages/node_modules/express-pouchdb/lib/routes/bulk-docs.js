...
module.exports = function (app) {
  ...
  app.post('/:db/_bulk_docs', utils.jsonParser, (req, res) => {
    ...
    let opts = 'new_edits' in req.body ?
    ...
    req.db.bulkDocs(req.body, opts, (err, response) => {
      if (err) {
        return utils.sendError(res, err);
      }
      utils.sendJSON(res, 201, response.map(info => {
        if (info.error) {
          return {
            id: info.id,
            error: info.name,
